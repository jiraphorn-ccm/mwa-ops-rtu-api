...
SET name = CASE
        WHEN @name_do_update::boolean THEN @name::varchar
        ELSE name
    END,
    equipment_type = CASE
        WHEN @equipment_type_do_update::boolean THEN sqlc.narg('equipment_type')::varchar
        ELSE equipment_type
    END,
    manufacturer = CASE
        WHEN @manufacturer_do_update::boolean THEN sqlc.narg('manufacturer')::varchar
        ELSE manufacturer
    END,
    brand = CASE
        WHEN @brand_do_update::boolean THEN sqlc.narg('brand')::varchar
        ELSE brand
    END,
    model = CASE
        WHEN @model_do_update::boolean THEN sqlc.narg('model')::varchar
        ELSE model
    END,
    serial_number = CASE
        WHEN @serial_number_do_update::boolean THEN sqlc.narg('serial_number')::varchar
        ELSE serial_number
    END,
    calibration_date = CASE
        WHEN @calibration_date_do_update::boolean THEN sqlc.narg('calibration_date')::date
        ELSE calibration_date
    END,
    expire_date = CASE
        WHEN @expire_date_do_update::boolean THEN sqlc.narg('expire_date')::date
        ELSE expire_date
    END,
    active = CASE
        WHEN @active_do_update::boolean THEN @active::boolean
        ELSE active
    END,
